...
export const stripeController = {
  ...
  async createCheckoutSession(req: Request, res: Response) {
    try {
      ...
      res.status(200).json({ sessionId: session.id, url: (session as any).url });
    } catch (error: any) {
    ...
    try {
      switch (event.type) {
        case 'checkout.session.completed': {
          ...
          const subscription = await (stripe as any).subscriptions.retrieve(session.subscription as string);
        ...
        case 'customer.subscription.deleted': {
          ...
          const userId = await subscriptionService.getUserIdByStripeCustomerId(subscription.customer as string);
